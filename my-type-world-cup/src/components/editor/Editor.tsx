...
const Editor = ({ setIsNumber }: EditorProps) => {
  ...
  const saveNewWorldcup = async (post_body: Post_req) => {
    try {
      const res = await post_worldcup(accessToken!, post_body!);
      resetAndNavigateToNextStep(res); // 공통 로직 호출
    } catch (err) {
      console.log(err);
    }
  };

  const updateExistingWorldcup = async (post_body: Post_req) => {
    try {
      const res = await patch_worldcup(accessToken!, post_body!, worldcup!.id);
      resetAndNavigateToNextStep(res); // 공통 로직 호출
    } catch (err) {
      console.log(err);
    }
  ...
  const resetAndNavigateToNextStep = (res: Post_res) => {
    setWorldcup(res); // 저장
    setTitle(""); // 초기화
    setDescription(""); // 초기화
  ...
  const handleSave = () => {
    ...
    if (accessToken !== null && !!title && !!description) {
      ...
      if (worldcup === null) {
        saveNewWorldcup(post_body);
      } else {
        updateExistingWorldcup(post_body);
      }
    }
  };
